...
{ 
	...
	if(get_timestamp($date_exp) < get_timestamp($_POST['date_exp']))
	{
	...
	// ----------------------------------------------------------------------------
		...
		$sql = 'UPDATE customer 
				...
				SET
					...
					date_exp			= DATE_ADD(date_exp, INTERVAL 30 DAY),
...
<div id="container">
	...
	<div id="content">
		...
		<form method="post" action="">
			...
			<input id="date_start" name="date_start" type="text" value="<?php echo date('Y-m-d') ?>" readonly="readonly" />
			<label for="date_exp">วันหมดอายุ : </label>
			<input name="date_exp" type="text" id="date_exp" value="<?php echo $date_exp = date('Y-m-d', strtotime(date('Y-m-d').'+1 month')).'"' ?>" readonly="readonly" />
			<label class="center">
				<input id="submit" name="submit" type="submit" value="บันทึก" />
			</label>
			<hr />
			<a href="customer_refill_credit_search.php">กลับ</a>
		</form>
	</div>
