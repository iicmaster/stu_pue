...
$(function(){
	...
	{
		var url = 'queue_wash.php?date=' + $(this).val();
			...
			while($data = mysql_fetch_array($query))
			{
				...
				for($loop = 1; $loop <= $total_machine ; $loop++)
				{
					...
					$sql_queue = 'SELECT 
								  ...
								  ON queue_wash.id_queue_time = queue_time.id
								  
								  LEFT JOIN order_head
								  ON queue_wash.id_order = order_head.id
								  ...
								  WHERE 
								 	 queue_wash.id_machine = "'.$id_machine[$loop - 1].'"
									 AND 
									 queue_wash.id_queue_time = "'.$data['id'].'"
									 AND 
									 DATE(queue_wash.date_start) = "'.$queue_date.'"';
					...
					if($query_queue_rows > 0)
					{
						...
						if($id_order[$id_machine[($loop - 1)]][$id_queue - 1] != $data_queue['id_order'])
						{
							...
							echo '<td rowspan="4" valign="middle">'.$queue_card.'</td>';
						}
					}
					else
					{
